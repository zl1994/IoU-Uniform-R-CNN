...
@DETECTORS.register_module
class CascadeRCNN_IoU(BaseDetector, RPNTestMixin):
    ...
    def __init__(self,
        ...
        assert bbox_roi_extractor is not None
        assert bbox_head is not None
        super(CascadeRCNN_IoU, self).__init__()
        ...
        if IoU_head is not None:
            self.IoU_roi_extractor = nn.ModuleList()
            self.IoU_head = nn.ModuleList()
            if not isinstance(IoU_roi_extractor, list):
                IoU_roi_extractor = [
                    IoU_roi_extractor for _ in range(num_stages)
                ]
            if not isinstance(IoU_head, list):
                IoU_head = [IoU_head for _ in range(num_stages)]
            assert len(IoU_roi_extractor) == len(IoU_head) == self.num_stages
            for roi_extractor, head in zip(IoU_roi_extractor, IoU_head):
                self.IoU_roi_extractor.append(
                    builder.build_roi_extractor(roi_extractor))
                self.IoU_head.append(builder.build_head(head))
    ...
    def init_weights(self, pretrained=None):
        super(CascadeRCNN_IoU, self).init_weights(pretrained)
        self.backbone.init_weights(pretrained=pretrained)
        if self.with_neck:
            if isinstance(self.neck, nn.Sequential):
                for m in self.neck:
        ...
        for i in range(self.num_stages):
            if self.with_bbox:
                ...
                self.bbox_head[i].init_weights()
            if self.with_IoU:
                self.IoU_roi_extractor[i].init_weights()
                self.IoU_head[i].init_weights()
            if self.with_mask:
                if not self.share_roi_extractor:
                    self.mask_roi_extractor[i].init_weights()
                self.mask_head[i].init_weights()
        ...
        for i in range(self.num_stages):
            ...
            if self.with_bbox or self.with_mask:
                ...
                for j in range(num_imgs):
                    ...
                    if self.with_IoU:
                        if i == 0:
                            IoU_bbox_sample, IoU_targets, gt_indexes, bbox_targets = bbox_assigner.assign_IoU(gt_bboxes[j],
                                                                                                              img_meta[j],
                                                                                                              reg_sample=True)
                        elif i == 1:
                            IoU_bbox_sample, IoU_targets, gt_indexes, bbox_targets = bbox_assigner.assign_IoU_2(gt_bboxes[j],
                                                                                                                img_meta[j],
                                                                                                                reg_sample=True)
                        else:
                            IoU_bbox_sample, IoU_targets, gt_indexes, bbox_targets = bbox_assigner.assign_IoU_3(gt_bboxes[j],
                                                                                                                img_meta[j],
                                                                                                                reg_sample=True)
            ...
            if self.with_IoU:
                ...
                for k in range(len(gt_indexes_results)):
                    gt_indexes_results[k] = 100 * k + gt_indexes_results[k]
                IoU_targets = torch.cat(IoU_targets_results, 0)
                gt_indexes = torch.cat(gt_indexes_results, 0)
                gt_bbox_targets = torch.cat(bbox_targets_results, 0)
                # TODO: a more flexible way to decide which feature maps to use
                IoU_roi_extractor = self.IoU_roi_extractor[i]
                IoU_head = self.IoU_head[i]

                IoU_bbox_feats = IoU_roi_extractor(
                    x[:IoU_roi_extractor.num_inputs], IoU_rois)
                if self.with_shared_head:
            ...
            if i < self.num_stages - 1:
                ...
                roi_labels = bbox_targets[0]  # bbox_targets is a tuple
                with torch.no_grad():
                    _, bbox_pred_IoUbranch = IoU_head(bbox_feats)
                    proposal_list = bbox_head.refine_bboxes(
                        rois, roi_labels, bbox_pred_IoUbranch, pos_is_gts, img_meta)

        return losses

    def simple_test(self, img, img_meta, proposals=None, rescale=False, gt_bboxes=None, gt_labels=None):
        x = self.extract_feat(img)
        proposal_list = self.simple_test_rpn(
            x, img_meta, self.test_cfg.rpn) if proposals is None else proposals
        ...
        for i in range(self.num_stages):
            ...
            IoU_pred, bbox_pred = IoU_head(bbox_feats)
            ms_scores.append(cls_score)
            ms_IoUs.append(IoU_pred)
        ...
        bboxes = rois
        IoU_roi_extractor = self.IoU_roi_extractor[-1]
        bboxes_feats = IoU_roi_extractor(
            x[:len(IoU_roi_extractor.featmap_strides)], bboxes)
        if self.with_shared_head:
            bboxes_feats = self.shared_head(bboxes_feats)
        IoU_head = self.IoU_head[-1]
        IoU_pred, bbox_pred = IoU_head(bboxes_feats)
        ms_IoUs.append(IoU_pred)
        '''
        cls_score_mean = sum(ms_scores) / self.num_stages
        IoU_pred_mean = sum(ms_IoUs[1:]) / self.num_stages
        '''
        cls_score_mean = sum(ms_scores) / self.num_stages
        IoU_pred = ms_IoUs[-1]
    ...
    def show_result(self, data, result, **kwargs):
        if self.with_mask:
            ...
            if isinstance(result, dict):
                result = result['ensemble']
        super(CascadeRCNN_IoU, self).show_result(data, result, **kwargs)